...
/**
 * Tampeymonkey function that opens a new window tab
...
/**
 * tampermonkey function that saves value to local memory
 * @param {string} the name of the variable to save the value too
 * @param {bool} value of variable parameter
 */
...
/**
 * tampermonkey function that gets the value of a given variable
 * Value will default to 'false' if no variable name found in local memory
 * @param {string} name of the variable to look up in local memory
 * @retunr {bool} the value of the defined variable
 */
...
/**
 * tampermonkey function that retrieves resource value defined in meta
 * @param {string} name of value in userscript meta
...
/**
 * tampermonkey function that retrieves all stored varaible names
 * @return {obj} Array of all variabled names and values that is stored in memory
 */
...
(function () {
    ...
    var launchToolbar = {
        ...
        'createElements': function () {
            launchToolbar.config = {
                ...
                '$copyFolderPath': jQuery('<div>').attr({
                    'class': 'funcButtons copyFolderPath click-able myClass',
        ...
        'cacheDOM': function () {
            ...
            // secondary owner
            this.$secondarylaunchOwner = jQuery('#CF00N40000002b62E_ileinner a').attr({
                'class': 'backupLaunchOwner',
            });
            this.secondaryLaunchOwner = jQuery('#CF00N40000002b62E_ileinner a');
            this.secondaryLaunchOwnerName = this.secondaryLaunchOwner.text();
            // proof link
            this.$proofLink = jQuery('#00N330000038wt0_ileinner');
        },
        /**
        ...
        'bindTargetPanelActions': function (currentTarget, callingPanel) {
            ...
            } else if (currentTarget.className.indexOf('backupLaunchOwner') > -1 && callingPanel.indexOf('email') > -1) {
                this.getEmail(this.$secondarylaunchOwner.attr('onmouseover'), this.$launchOwner.attr('onmouseover'));
            ...
            } else if (currentTarget.className.indexOf('backupLaunchOwner') > -1 && callingPanel.indexOf('logActivity') > -1) {
                this.activityLog(this.$secondarylaunchOwner.attr('onmouseover'));
        ...
        /**
         * Will remove self from the email and log panels
         */
        ...
        /**
         * Build panel members and
         * Remove elements that are empty
         */
        'buildLaunchMembersPanel': function ($panel) {
            // create strings for inserting into panel
            var launchOwner = this.launchOwnerText.trim() === '' ? '' : 'IC: ' + this.launchOwnerText;
            var backupLaunchOwner = this.secondaryLaunchOwnerName.trim() === '' ? '' : 'Secondary IC: ' + this.secondaryLaunchOwnerName;
            var designerName = this.designerName.trim() === '' ? '' : 'WDS: ' + this.designerName;
            var preDesigner = this.preDesignName.trim() === '' ? '' : 'PD: ' + this.preDesignName;
            var qaDesigner = this.qaDesignerName.trim() === '' ? '' : 'QA: ' + this.qaDesignerName;

            // add names to panel
            $panel.append(jQuery(`<div class="myClass launchMembers ic">${launchOwner}</div>`))
                .append(jQuery(`<div class="myClass launchMembers backupLaunchOwner">${backupLaunchOwner}</div>`))
                .append(jQuery(`<div class="myClass launchMembers designer">${designerName}</div>`))
                .append(jQuery(`<div class="myClass launchMembers preDesign">${preDesigner}</div>`))
                .append(jQuery(`<div class="myClass launchMembers qaDesigner">${qaDesigner}</div>`));
        ...
        'getEmail': function (emailTarget, primaryEmail) {
            ...
            // if secondary owner is chosen: get primary owner email, then secondary owner email
            if (primaryEmail) {
                jQuery.ajax({
                    'url': primaryEmail.split("'")[3], // eslint-disable-line quotes
                        ...
                        jQuery.ajax({
                            ...
                            'success': function (data) {
                                ...
                                launchToolbar.config.$salesforceEmailOwner.attr({
                                    'onclick': window.location.href = encodeURI(`${newEmailLink}&p4=${managerEmails}&p6=${self.comboID}&retURL=${retURL}&p24=${emailAddress}`),
                                });
                            },
                        });
                    },
        ...
        'bindEvents': function () {
            ...
            // secondary owner
            this.$secondarylaunchOwner.on('mousedown', this.doWork.bind(this));
            this.$secondarylaunchOwner.bind('contextmenu', function () {
                return false;
            });
        ...
        'addFunctionStyles': function () {
            launchToolbar.config.$toolbarStyles
                ...
                .append('.activeFunction, .myClass:hover { background: teal; color: white; }')
                .append('.targetsPanel { display: none; position: absolute; background: linear-gradient(to right, rgb(178, 254, 250), rgb(14, 210, 200)); margin-top: 38px; margin-left: 1305px; border-left: 1px solid rgb(0, 0, 0); border-bottom: 1px solid rgb(0, 0, 0); border-right: 1px solid rgb(0, 0, 0) }');
        },
        'addOptionStyles': function () {
            launchToolbar.config.$toolbarStyles
                ...
                .append('.proofLink { background: #32cd32; } ');
        },
        ...
        'makeLarger': function () {
            ...
            this.$proofLink.toggleClass('makeLarger');
        },
        'colorizeLaunch': function () {
            ...
            this.$proofLink.toggleClass('hFont proofLink');
        },
        'startTool': function () {
        ...
        'doWork': function (event) {
            ...
            switch (true) {
                ...
                case event.which === 3 && classText.indexOf('launchOwner') > -1:
                    ...
                    break;
                case event.which === 3 && classText.indexOf('backupLaunchOwner') > -1:
                    this.copyInfo(this.secondaryLaunchOwnerName);
                    break;
            }
        },
